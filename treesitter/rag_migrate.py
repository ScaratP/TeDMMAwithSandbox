...
def generate_test_case_by_BM25():
    ...
    while True:
        ...
        try:
            ...
            # 3. 建立專屬該微服務的輸出資料夾
            import os
            output_dir = f"./generated_tests/{question.strip()}"
            os.makedirs(output_dir, exist_ok=True)

            if not karate_blocks and not pact_blocks:
                print(f"\n⚠️ LLM 未依照格式輸出 Markdown 區塊。將儲存完整原始文字供除錯。")
                debug_file = os.path.join(output_dir, f"{question.strip()}_raw_output.txt")
            ...
            for idx, block in enumerate(karate_blocks):
                file_path = os.path.join(output_dir, f"{question.strip()}_api_test_{idx+1}.feature")
            ...
            for idx, block in enumerate(pact_blocks):
                file_path = os.path.join(output_dir, f"{question.strip()}_contract_{idx+1}.json")
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(block.strip())
